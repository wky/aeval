...
namespace ufo
{
  ...
  void enumConstPredForEachVar(ExprVector& vars, unsigned bw_bound, vector<ExprVector>& lol){
    for (Expr v : vars){
      ...
      unsigned real_bw = bv::width(v->first()->arg(1));
      unsigned bw = real_bw;
      if (bw > bw_bound) bw = bw_bound;
      for (unsigned i = 0; i < (1ul<<bw); i++){
        Expr pred = mk<EQ>(v, bv::bvnum(i, real_bw, v->efac()));
  ...
  void enumDataPredForEachVar(ExprVector& LHSvars, unsigned bw_bound, ExprVector& RHSvars, vector<ExprVector>& lol){
    for (Expr v : LHSvars){
      ...
      if (bw > bw_bound) bw = bw_bound;
      for (unsigned i = 0; i < (1ul<<bw); i++){
        Expr pred = mk<EQ>(v, bv::bvnum(i, bw, v->efac()));
        preds.push_back(pred);
      }

      for (Expr rhs: RHSvars) preds.push_back(mk<EQ>(v, rhs));
      // ADD
      for (int i = 0; i < RHSvars.size(); i++)
        for (int j = i + 1; j < RHSvars.size(); j++)
          preds.push_back(mk<EQ>(v, mk<BADD>(RHSvars[i], RHSvars[j])));
